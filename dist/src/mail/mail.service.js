...
const capitalize_word_util_1 = require("../common/utils/capitalize-word.util");
let MailService = class MailService {
    mailerConfiguration;
    transporter;
    constructor(mailerConfiguration) {
    ...
    async sendResetPasswordSuccessEmail(name, to) {
        const mailOptions = {
            from: `"Koonage Infotec Team" <${this.mailerConfiguration.user}>`,
            to,
    ...
    async sendAccountCreationEmail(to, firstName, email, role, tempPassword) {
        const mailOptions = {
            from: `"Koonage Infotec Team" <${this.mailerConfiguration.user}>`,
            to,
            subject: `Your ${(0, capitalize_word_util_1.capitalizeWords)(role)} Calibration System Account Has Been Created`,
            html: `
    ...
    async sendClientCreationEmail(to, firstName, email, role, password) {
        const mailOptions = {
            from: `"Koonage Infotec Team" <${this.mailerConfiguration.user}>`,
            to,
            subject: `Your ${(0, capitalize_word_util_1.capitalizeWords)(role)} Calibration System Account Has Been Created`,
    ...
    async sendVerificationCodeEmail(to, firstName, code) {
        const mailOptions = {
            from: `"Koonage Infotec Team" <${this.mailerConfiguration.user}>`,
            to,
    ...
    async sendWelcomeEmail(to, firstName, role) {
        const mailOptions = {
            from: `"Koonage Infotec Team" <${this.mailerConfiguration.user}>`,
            to,
            subject: `Your ${(0, capitalize_word_util_1.capitalizeWords)(role)} Calibration System Account Has Been Created`,
      ...
      <body style="margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; background-color: #f5f7fa; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 12px rgba(0,0,0,0.06);">
          ...
          <div style="background: linear-gradient(135deg, #1e40af, #3b82f6); padding: 50px 30px; text-align: center; color: #e0f2fe;">
            ...
            <p style="margin-top: 10px; font-size: 16px;">Hi ${firstName}, your ${(0, capitalize_word_util_1.capitalizeWords)(role)} account is now active.</p>
          </div>
...
exports.MailService = MailService = __decorate([
    ...
    __param(0, (0, common_1.Inject)(mailer_config_1.default.KEY)),
